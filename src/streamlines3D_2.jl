...
"""
@recipe(StreamLines3D, limits) do scene
    attr = Attributes(
        stepsize = 0.01,
        gridsize = (32, 32, 32),
        maxsteps = 500,
...
function convert_arguments(::Type{<: StreamLines3D}, xrange, yrange, zrange)
...
function plot!(p::StreamLines3D)
    data = lift(p, p.limits, p.gridsize, p.stepsize, p.maxsteps, p.density, p.color) do limits, resolution, stepsize, maxsteps, density, color_func
