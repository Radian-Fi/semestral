...
function visualize_main(elbound, elfield, elbhcenter, elbhr, magbound, magfield, magbhcenter, magbhr)
  ...
  color=norm, colormap=:viridis, gridsize=(16, 16, 16),  arrow_size=convert(Float64, 2*elbound[1].step), linewidth=convert(Float64, 100*elbound[1].step),
  transparency=true)
  ...
  elvolumedata = divergence(elfield, convert(Float64, elbound[1].step))
  elmindiv, elmaxdiv = extrema(elvolumedata)
  elvolumedata = Float32.((elvolumedata .- elmindiv) ./ (elmaxdiv - elmindiv))
  volume!(axs[1], elbound[1], elbound[2], elbound[3], elvolumedata, algorithm=:mip, alpha=1.0, colormap=volumecolormap)
  ...
  color=norm, colormap=:viridis, gridsize=(16, 16, 16), arrow_size=convert(Float64, 2*magbound[1].step), linewidth=convert(Float64, 100*magbound[1].step),
  transparency=true)
  ...
  magminnorm, magmaxnorm = extrema(magvnorm)
  # magnorm = (magvnorm .- magminnorm) ./ (magmaxnorm - magminnorm)
  clamp!(magvnorm, 0, 0.5)
  magnorm = magvnorm ./ 0.5
  ...
  volume!(axs[2], magbound[1], magbound[2], magbound[3], magvolumedata, algorithm=:mip, alpha=1.0, colormap=volumecolormap)
  # volume!(axs[2], magbound[1], magbound[2], magbound[3], (1 .- magnorm), algorithm=:mip, alpha=1.0, colormap=normcolormap)

  magmindiv, magmaxdiv = extrema(magvolumedata)
  println((magminnorm, magmaxnorm))
  println(magnorm[ceil(Int64, (0.723-magbound[1][begin])/convert(Float64, magbound[1].step)), ceil(Int64, (-1.117-magbound[2][begin])/convert(Float64, magbound[2].step)), ceil(Int64, (-0.97-magbound[3][begin])/convert(Float64, magbound[3].step))])
  fig = link_cameras_lscene(fig)
  fig
end;
...
function run_main(filename1="/media/radian-fi/KINGSTON/VIZ/Blackhole/el3_512_512_512.csv", filename2="/media/radian-fi/KINGSTON/VIZ/Blackhole/mag3_512_512_512.csv", subsample=4)
  elbound, elfield, elbhcenter, elbhr = read_dataset("/media/radian-fi/KINGSTON/VIZ/Blackhole/el3_512_512_512.csv", subsample)
  magbound, magfield, magbhcenter, magbhr = read_dataset("/media/radian-fi/KINGSTON/VIZ/Blackhole/mag3_512_512_512.csv", subsample)
  println(elbound)
